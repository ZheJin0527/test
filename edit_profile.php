...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '') {
        ...
        $stmt = $conn->prepare("UPDATE users SET username = ?, phone_number = ?, bank_name = ?, bank_account = ? WHERE id = ?");
        $stmt->bind_param("ssssi", $username, $phone, $bank_name, $bank_account, $user_id);
        if ($stmt->execute()) {
            $success = "更新成功！";
            $_SESSION['username'] = $username; // 同步 session
        } else {
            $error = "更新失败，请稍后再试。";
        }
        $stmt->close();
...
// 读取用户当前资料
$stmt = $conn->prepare("SELECT username, email, phone_number, bank_name, bank_account, position FROM users WHERE id = ?");
