...
<body>
    <div class="container">
        ...
        <div class="filter-section">
            ...
            <div class="filter-actions">
                ...
                <button class="btn btn-primary" onclick="printSampleData()" style="background-color: #8b5cf6;">
    ...
    <script>
        ...
        function printSampleData() {
            // 询问要打印多少条示例数据
            const count = prompt('请输入要打印的示例数据数量（1-50）：', '10');
            ...
            // 生成示例数据
            const sampleData = generateSampleDataArray(parseInt(count));
            
            // 创建打印内容
            const printContent = createPrintContent(sampleData);
        ...
        // 生成示例数据数组
        function generateSampleDataArray(count) {
            ...
            const sampleReceivers = ['张三', '李四', '王五', '赵六', '陈七', '刘八', '杨九', '黄十'];
            const sampleRemarks = ['正常进货', '紧急补货', '促销活动', '节日特供', '新品上架', '库存调整'];
            
            const data = [];
            
            for (let i = 0; i < count; i++) {
                const product = sampleProducts[Math.floor(Math.random() * sampleProducts.length)];
                const receiver = sampleReceivers[Math.floor(Math.random() * sampleReceivers.length)];
                const remark = sampleRemarks[Math.floor(Math.random() * sampleRemarks.length)];
                
                // 随机生成数量（1-20）
                const quantity = Math.floor(Math.random() * 20) + 1;
                const unitPrice = product.price + (Math.random() - 0.5) * 10; // 价格波动
        ...
        function createPrintContent(data) {
            ...
            const restaurantName = getRestaurantName(currentStockType);
            ...
            return `
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>示例数据打印 - ${restaurantName}</title>
                    <style>
                        body {
                ...
                <body>
                    <div class="print-content">
                        <div class="print-header">
                            <h1>${restaurantName} - 示例库存数据</h1>
                            <p>生成时间: ${currentDate} | 数据条数: ${data.length}</p>
                        </div>
        ...
        // 获取餐厅名称
        function getRestaurantName(stockType) {
            const names = {
                'j1': 'J1分店',
                'j2': 'J2分店', 
                'j3': 'J3分店',
