...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="controls">
                <button class="btn btn-info" onclick="openMinStockModal()">
    ...
    <!-- 最低库存设置模态框 -->
    <div id="min-stock-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>最低库存设置</h3>
                <button class="modal-close" onclick="closeMinStockModal()">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="modal-body">
                <div class="filter-group">
                    <label for="global-min-stock">全局最低库存数量</label>
                    <input type="number" id="global-min-stock" class="filter-input" 
                        placeholder="设置所有产品的最低库存" min="0" step="1">
                    <button class="btn btn-primary" onclick="setGlobalMinStock()" style="margin-left: 10px;">
                        应用到所有产品
                    </button>
                </div>
                
                <h4 style="margin-top: 30px;">单个产品设置</h4>
                <div class="filter-group">
                    <input type="text" id="product-search" class="filter-input" 
                        placeholder="搜索产品名称..." onkeyup="searchProducts()">
                </div>
                
                <div class="min-stock-list" id="min-stock-list">
                    <!-- 动态生成产品列表 -->
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-success" onclick="saveMinStockSettings()">
                    <i class="fas fa-save"></i>
                    保存设置
                </button>
                <button class="btn btn-secondary" onclick="closeMinStockModal()">
                    取消
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        async function openMinStockModal() {
            document.getElementById('min-stock-modal').style.display = 'block';
            await loadProductsForMinStock();
        }

        // 关闭最低库存设置模态框
        function closeMinStockModal() {
            document.getElementById('min-stock-modal').style.display = 'none';
        }

        // 加载产品列表用于最低库存设置
        async function loadProductsForMinStock() {
            try {
                // 从stockData中获取唯一产品
                ...
                stockData.forEach(item => {
                    const key = `${item.product_name}_${item.product_code}`;
                    if (!seen.has(key)) {
                        seen.add(key);
                        uniqueProducts.push({
                ...
                await loadMinStockSettings();
                renderProductsList();
                
            } catch (error) {
                showAlert('加载产品列表失败: ' + error.message, 'error');
            }
        }

        // 加载最低库存设置
        async function loadMinStockSettings() {
            try {
                const response = await fetch(`${API_BASE_URL}?action=get_min_stock`);
                const result = await response.json();
                
                if (result.success) {
                    minStockSettings = result.data || {};
                }
            } catch (error) {
                console.error('加载最低库存设置失败:', error);
                minStockSettings = {};
            }
        }
        ...
        function renderProductsList() {
            const container = document.getElementById('min-stock-list');
            
            if (productsList.length === 0) {
                container.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">暂无产品数据</div>';
                return;
            }
            
            let html = '';
            productsList.forEach(product => {
                const key = `${product.product_name}_${product.product_code}`;
                const minStock = minStockSettings[key] || 0;
                
                html += `
                    <div class="min-stock-item">
                        <div class="product-info">
                            <div class="product-name">${product.product_name}</div>
                            <div class="product-code">${product.product_code}</div>
                        </div>
                        <input type="number" class="min-stock-input" 
        ...
        function searchProducts() {
            ...
            const items = document.querySelectorAll('.min-stock-item');
        ...
        // 设置全局最低库存
        function setGlobalMinStock() {
            const globalValue = document.getElementById('global-min-stock').value;
            
            if (!globalValue || parseFloat(globalValue) < 0) {
                showAlert('请输入有效的库存数量', 'error');
                return;
            }
            ...
            showAlert('全局最低库存已设置', 'success');
        }

        // 保存最低库存设置
        async function saveMinStockSettings() {
            try {
                const settings = {};
                const inputs = document.querySelectorAll('.min-stock-input');
                ...
                if (result.success) {
                    minStockSettings = settings;
                    showAlert('最低库存设置已保存', 'success');
                    closeMinStockModal();
                } else {
